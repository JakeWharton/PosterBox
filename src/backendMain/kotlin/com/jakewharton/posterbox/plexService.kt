...
class HttpPlexService(
  ...
  override suspend fun posters(): List<Poster> {
    ...
    val sections = json.decodeFromString(
      PlexResponse.serializer(PlexSections.serializer()),
      sectionsJson,
    )

    return sections.mediaContainer.sections
      .filter { it.type == "movie" || it.type == "show" }
      .filter { config.libraries == null || it.title in config.libraries }
      .flatMap { sectionHeader ->
        val sectionUrl = URLBuilder(config.host)
          .takeFrom("/library/sections/${sectionHeader.key}/all")
          .build()
        ...
        val section = json.decodeFromString(
          PlexResponse.serializer(PlexItems.serializer()),
          sectionJson,
        )
...
private data class PlexSections(
  @SerialName("Directory") val sections: List<SectionHeader>,
) {
  @Serializable
...
private data class PlexItems(
  @SerialName("Metadata") val items: List<PlexItem>,
)
...
private data class PlexItem(
  ...
  @SerialName("duration") val durationMillis: Int,
